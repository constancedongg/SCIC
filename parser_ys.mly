...
%token METER SEC KGRAM AMP CMETER HERTZ GRAM NEWTON NOUNIT UID UONE
...
func_decl: 
	| type unit FUNC ID formals_block stmt_block 
	{{
		return_type       = $1;
		return_unit       = $2;
		func_identifier = $4;
	...
	| type FUNC ID formals_block stmt_block 
	{{
		return_type       = $1;
		return_unit		  = NOUNIT;
		func_identifier = $3;
...
equa_decl:	
	| EQUA ID formals_block LBRACE stmt RBRACE 
	{{
		equa_identifier = $2;
...
/* formals Block */
formals_block: LPAREN opt_formals RPAREN { $2 }
...
formals_list_unit: 
		| typ unit ID             { [($1, $2, $3)] }
        | aug_list COMMA typ unit ID  { ($3, $4, $5) :: $1 }

formals_list_no_unit:
		| typ ID             { [($1, NOUNIT, $2)] }
		| aug_list COMMA typ ID 	  { ($3, NOUNIT, $4)} /* no unit - how to enforce none has unit? */
...
unit_decl:
	| BAR PRIME LBRACE UID RBRACE ASN cexpr TIMES unit {{
		uid = $4;
		c_expr = $7;
		u_expr = $9; }}
...
uexpr:
	...
	| uexpr POW INT_LITERAL 		{ Binop($1, UPow, $3) }
